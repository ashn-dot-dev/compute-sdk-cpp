...
bool Request::is_from_client() {
    return this->req->is_from_client();
}
...
Response Request::send(std::string backend) {
    Response res{fastly::sys::http::m_http_request_send(std::move(this->req), std::move(backend))};
    return res;
}
...
void Request::set_body(std::vector<uint8_t> body) {
    this->req->set_body(std::move(body));
}

void Request::set_body(std::string body) {
    this->set_body(std::move(std::string(body.begin(), body.end())));
}

Request* Request::with_body(std::vector<uint8_t> body) {
...
bool Request::has_body() {
    return this->req->has_body();
}

std::unique_ptr<std::vector<uint8_t>> Request::into_body_bytes() {
    return fastly::sys::http::m_http_request_into_body_bytes(std::move(this->req));
}
